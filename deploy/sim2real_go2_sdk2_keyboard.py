...
class Go2SDK2Controller:
    ...
    This keeps RX-00's observation/action logic and replaces only the old
    unitree_legged_sdk UDP transport with SDK2 DDS transport.

    SDK motor order used here:
      FR_0, FR_1, FR_2,
      FL_0, FL_1, FL_2,
      RR_0, RR_1, RR_2,
      RL_0, RL_1, RL_2
    ...
    def release_motion_mode(self):
        """
        Release Unitree sport/high-level mode before low-level DDS control.

        This is the missing step that go2_ctrl was effectively doing before.
        Without this, low-level commands can fight the active motion service.
        """
        ...
        try:
            while True:
                ...
                if sim_time <= self.config.standup_duration:
                    ...
                    self.send_command(
                        self.qDes_train,
                        self.config.kp_stand,
                        self.config.kd_stand,
                    )
...
def main():
    ...
    parser.add_argument(
        "--network",
        required=True,
        help="Robot Ethernet interface, e.g. enp130s0",
    )
